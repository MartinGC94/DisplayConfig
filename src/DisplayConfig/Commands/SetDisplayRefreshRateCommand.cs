...
{
    [Cmdlet(VerbsCommon.Set, "DisplayRefreshRate", DefaultParameterSetName = "ApplyNow")]
    [OutputType(typeof(API.DisplayConfig), ParameterSetName = new string[] { "Config" })]
    public sealed class SetDisplayRefreshRateCommand : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            foreach (uint id in DisplayId)
            {
                try
                {
                    configToModify.SetDisplayRefreshRate(id, RefreshRate);
                }
                catch (Exception error) when (!(error is PipelineStoppedException))
                {
                    WriteError(new ErrorRecord(error, "SetRefreshRateError", Utils.GetErrorCategory(error), id));
                }
            }
